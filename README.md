...
By default, JAX-RS resource classes follow a per-request lifecycle. This means the runtime creates a new instance of the resource class for every incoming HTTP request, and that instance is discarded once the response is sent. To make a resource class behave as a singleton (one shared instance for all requests), you would need to explicitly annotate it with @Singleton or use a CDI scope like @ApplicationScoped.
This lifecycle decision has a direct impact on how we manage in-memory data. Because each request gets its own resource instance, instance variables cannot be used to store shared state across requests. If we declared our rooms list or sensors map as instance variables, each new request would get a fresh, empty copy and any data added by a previous request would be lost. To share data across requests, we need to store it at a higher scope, for example in a static field or a separate singleton class that all resource instances can reference.
This then introduces the thread safety concern, since multiple requests can run concurrently and all reference the same shared data structure. Without protection, two threads could modify a list simultaneously and cause data corruption or inconsistent results. In a production system, we would address this using synchronized blocks or thread-safe collections like CopyOnWriteArrayList or ConcurrentHashMap. For this coursework, with low test traffic and simple operations, we kept things straightforward, but the risk is worth acknowledging.
